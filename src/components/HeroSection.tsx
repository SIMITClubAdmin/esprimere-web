...
export default function HeroSection() {
    return (
        <section className="relative h-[110vh] md:h-[115vh] w-full bg-gradient-to-b from-[var(--color-cream-1)] to-[var(--color-brown-3)] overflow-hidden">
            <style jsx>{`
              @keyframes fadeInOut {
                0%, 100% { opacity: 0.2; }
                50% { opacity: 0.8; }
            ...
            {/* Club Name & Tagline */}
            <div className="absolute top-[40%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 text-center px-4 w-full max-w-7xl">
                <h1 className="font-saint-pauline text-4xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-[10rem] font-extralight text-[var(--color-brown-1)] tracking-wide leading-tight">
                    <span className="block sm:inline">Sim</span>{' '}
                    <span className="block sm:inline">Esprimere</span>
                </h1>
                <p className="font-dream-avenue text-lg sm:text-xl md:text-3xl lg:text-4xl xl:text-5xl text-[var(--color-brown-1)] mt-2 md:-mt-4 leading-relaxed">
            ...
            <img
                ...
                className="w-full h-auto absolute bottom-0 left-0 z-20 md:hidden"
            ...
            <img
                ...
                className="w-full h-auto absolute bottom-0 left-0 z-20 hidden md:block"
                style={{ transform: 'translateY(50%)' }}
            />
        </section>
    );
